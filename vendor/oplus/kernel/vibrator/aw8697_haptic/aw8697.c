...
/* used for 7 end*/
    ...
    {"aw8697_reserved_364.bin"},
    {"aw8697_Threestagekey_RTP_365_168z.bin"},
    {"aw8697_reserved_366.bin"},
    {"aw8697_reserved_367.bin"},
    {"aw8697_reserved_368.bin"},
    {"aw8697_reserved_369.bin"},
...
static int aw8697_haptic_read_f0(struct aw8697 *aw8697)
{
	...
	if(ret < 0) {
		pr_err("%s: aw8697_i2c_read F0_H error ret %d\n", __func__, ret);
	}
	f0_reg = (reg_val << 8);
	ret = aw8697_i2c_read(aw8697, AW8697_REG_F_LRA_F0_L, &reg_val);
	if(ret < 0) {
		pr_err("%s: aw8697_i2c_read F0_L error ret %d\n", __func__, ret);
	}
	f0_reg |= (reg_val << 0);
#ifndef OPLUS_FEATURE_CHG_BASIC
...
static int aw8697_haptic_read_cont_f0(struct aw8697 *aw8697)
{
	...
	if (ret < 0) {
		pr_err("%s: aw8697_i2c_read LRA_CONT_H error ret %d\n", __func__, ret);
	}
	f0_reg = (reg_val << 8);
	ret = aw8697_i2c_read(aw8697, AW8697_REG_F_LRA_CONT_L, &reg_val);
	if (ret < 0) {
		pr_err("%s: aw8697_i2c_read LRA_CONT_L error ret %d\n", __func__, ret);
	}
	f0_reg |= (reg_val << 0);
...
static int aw8697_haptic_read_beme(struct aw8697 *aw8697)
{
    ...
    if (ret < 0) {
        pr_err("%s: aw8697_i2c_read VOL_MP error ret %d\n", __func__, ret);
    }
    aw8697->max_pos_beme = (reg_val<<0);
    ret = aw8697_i2c_read(aw8697, AW8697_REG_WAIT_VOL_MN, &reg_val);
    if (ret < 0) {
        pr_err("%s: aw8697_i2c_read VOL_MN error ret %d\n", __func__, ret);
    }
...
static void aw8697_rtp_key_work_routine(struct work_struct *work)
{
	...
	case SG_INPUT_DOWN_HIGH:
		...
		} else if (aw8697->f0 < OPLUS_176HZ_F0) {
...
static int aw8697_haptic_get_f0(struct aw8697 *aw8697, unsigned int f0_pre)
{
    ...
    for(i=0; i<f0_cali_cnt; i++) {
        ...
        /* f0 calibrate done */
        if(reg_val & 0x01) {
            ret = aw8697_haptic_read_f0(aw8697);
...
{
	struct aw8697 *aw8697 = PDE_DATA(file_inode(filp));
	uint8_t ret = 0;
	int style = 0;
	char page[10];
...
static int init_vibrator_proc(struct aw8697 *aw8697)
{
	...
	if (prEntry_da == NULL) {
		ret = -ENOMEM;
		dev_err(aw8697->dev, "%s: Couldn't create vibrator proc entry, ret %d\n",
			  __func__, ret);
	}
	...
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		dev_err(aw8697->dev, "%s: Couldn't create proc entry, %d, ret %d\n", __func__,
			  __LINE__, ret);
	}
	return 0;
}
...
static ssize_t aw8697_livetap_support_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#ifdef TIMED_OUTPUT
    struct timed_output_dev *to_dev = dev_get_drvdata(dev);
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", aw8697->livetap_support);
}

static ssize_t aw8697_livetap_support_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
    struct timed_output_dev *to_dev = dev_get_drvdata(dev);
	...
	if (val > 0)
		aw8697->livetap_support = true;
	else
		aw8697->livetap_support = false;
...
static DEVICE_ATTR(livetap_support, S_IWUSR | S_IRUGO, aw8697_livetap_support_show, aw8697_livetap_support_store);
#endif
...
static int aw8697_parse_dt(struct device *dev, struct aw8697 *aw8697,
    ...
    aw8697->livetap_support = of_property_read_bool(np, "oplus,livetap_support");
    dev_info(dev, "oplus,livetap_support = %d\n", aw8697->livetap_support);
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
static void aw8697_i2c_remove(struct i2c_client *i2c)
{
