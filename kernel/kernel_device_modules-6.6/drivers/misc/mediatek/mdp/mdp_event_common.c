...
/*
 * op6893: these dts_name spellings use underscores, matching the original
 * 4.19 DTB this project boots.  cmdq_dev_init_event_table() reads each one
 * with of_property_read_u32(), which is a plain strcmp: the new-BSP hyphen
 * spelling ("mdp-rdma0-sof") never matches the DTB's "mdp_rdma0_sof", so the
 * lookup fails silently and every GCE hardware event keeps the placeholder
 * cmdq_core_init_dts_data() left behind (CMDQ_SYNC_TOKEN_INVALID - 1 - i).
 * cmdq_core_get_event_value() then returns negative for all of them, and
 * every CMDQ_MOP_CLEAR/WAIT/SET/ACQUIRE meta in an MDP job fails to
 * translate -- which is what kept HWC's blit path from ever starting.
 * The 4.19 driver's table is identical except for these spellings.
 */
